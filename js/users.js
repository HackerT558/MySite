(function(){
  ...
  const myRoleLevelMap = { 'user':1,'manager':2,'manager-top':3,'manager-general':4,'admin':5 };

  function openModal(){ modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); }
  function closeModal(){
    modal.classList.remove('open'); modal.setAttribute('aria-hidden','true');
    form.reset(); avatarImg.style.display='none'; avatarPH.style.display='inline';
    const roleSel = document.getElementById('f_role'); roleSel.disabled=false; roleSel.title='';
  }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (btn) {
      ...
      // блокировка селектора роли если редактируем себя или нет прав
      const roleSel = document.getElementById('f_role');
      if (isSelf || !canManage) {
        roleSel.disabled = true;
        roleSel.title = isSelf ? 'Нельзя менять роль самому себе' : 'Недостаточно прав для смены роли';
      } else {
        roleSel.disabled = false; roleSel.title = '';
      }

      if (data.avatar) { avatarImg.src = data.avatar; avatarImg.style.display='block'; avatarPH.style.display='none'; }
      else { avatarImg.style.display='none'; avatarPH.style.display='inline'; }
      openModal();
    }
  });
